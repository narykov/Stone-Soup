...
# def plot_detection3D(detection, ax='Detections', color='b', marker=None, label=None, mapping=None):
#     if not ax:
#         ax = plt.gca()
#     if mapping is None:
#         mapping = [0, 2, 4]
...
#         marker = 'x'
#     datum = detection.measurement_model.inverse_function(detection)
#     plt.scatter(datum[mapping[0]], datum[mapping[1]], datum[mapping[2]], color=color, marker=marker, label=label)
#     # station = detection.measurement_model.translation_offset
#     # plt.plot(station[0], station[1], color='k', marker='.')
...
def do_single_target_tracking(prior=None, predictor=None, updater=None, measurements=None):
    ...
    for i, measurement in enumerate(measurements):
        plot_detection(measurement, mapping=mapping, ax=ax)
        plt.pause(0.1)
    ...
    def detections_gen(self):
        ...
        with self.path.open(encoding=self.encoding, newline='') as csv_file:
            ...
            for row in csv.DictReader(csv_file, **self.csv_options):
                ...
                measurement_model.translation_offset = StateVector([
                    [float(row['XSTAT_X'])*1000],
                    [float(row['XSTAT_Y'])*1000],
                    [float(row['XSTAT_Z'])*1000]
                ])
                # measurement_model.translation_offset = StateVector([
                #     [0],
                #     [0],
                #     [0]
                # ])
                r = np.sqrt(float(row['XSTAT_X'])**2 + float(row['XSTAT_Y'])**2 + float(row['XSTAT_Z'])**2)
                measurement_model.rotation_offset = StateVector([
                    [np.arccos(float(row['XSTAT_X'])/r)],
                    [np.arccos(float(row['XSTAT_Y'])/r)],
                    [np.arccos(float(row['XSTAT_Z'])/r)]
                ])
...
def main():
    ...
    path = "src/csv/RR01_data_alex_good.csv"
    filter_value = 'RR01'
    ...
    state_vector_fields = ("ANGLE_2", "ANGLE_1", "RANGE")
    # state_vector_fields = ("ANGLE_1_GCRF", "ANGLE_2_GCRF", "RANGE_GCRF")
